...
export default function SessionAccessPage() {
  ...
  const handleCreate = async (event: React.FormEvent) => {
    ...
    try {
      ...
      persistSession({ sessionId: response.sessionId, name: tripName })
      toast.success('Session created and linked to this device.')
    } catch (error) {
      toast.error((error as { message?: string })?.message ?? 'Failed to create session.')
    } finally {
  ...
  const handleJoin = (event: React.FormEvent) => {
    ...
    persistSession({ sessionId: inviteCode.trim(), name: attendeeName })
    toast.success('Session linked. Jump to the dashboard!')
  }
        ...
        >
          {mode === 'create' ? (
            <Fragment>
              ...
              >
                {isSubmitting ? 'Creating…' : 'Create live itinerary'}
              </button>
            </Fragment>
          ) : (
            <Fragment>
